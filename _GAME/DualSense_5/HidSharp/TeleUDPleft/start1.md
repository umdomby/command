...
левый курок должен делать резкий толчек когда происходит блокировка колес Тормоз: = 0 , левый курок должен реагировать только на тормоз когда диапазон равен  (motion.m_brake * 100):F0 = 0


L2 должен отталкивать (максимально жёстко, 2 секунды, импульсами) — когда Тормоз = 0% в телеметрии.
НЕ при отпускании, НЕ при резком изменении — просто когда motion.m_brake == 0.
Максимальная сила. 2 секунды. Импульсы. Только при 0%.