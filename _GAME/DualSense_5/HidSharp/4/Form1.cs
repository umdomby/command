...
namespace DualSenseTriggerControl
{
    public partial class Form1 : Form
    {
        ...
        // --- НОВЫЕ ПЕРЕМЕННЫЕ СОСТОЯНИЯ ДЛЯ ХРАНЕНИЯ ТЕКУЩЕГО ЭФФЕКТА КУРКОВ ---
        ...
        // Метод для R2: устанавливает новый эффект R2 и сохраняет текущий эффект L2
        private void SendR2Effect(byte effectID, byte param1, byte param2, byte heavyRumble = 0x00, byte lightRumble = 0x00)
        {
            ...
            // Отправляем текущий эффект R2 и СОХРАНЕННЫЙ эффект L2
        ...
        // Метод для L2: устанавливает новый эффект L2 и сохраняет текущий эффект R2
        private void SendL2Effect(byte effectID, byte param1, byte param2, byte heavyRumble = 0x00, byte lightRumble = 0x00)
        {
            ...
            // Отправляем СОХРАНЕННЫЙ эффект R2 и текущий эффект L2
            SendDualSenseReportDual(
                _currentEffectID_R2, _currentParam1_R2, _currentParam2_R2,
                _currentEffectID_L2, _currentParam1_L2, _currentParam2_L2,
                heavyRumble, lightRumble
        ...
        // Метод для вибрации (Сбрасывает оба курка и запоминает сброс)
