...
namespace TeleUDP
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            {
                Properties.Settings.Default.ListenPort = 20778;
                Properties.Settings.Default.Save();
            }
            LISTEN_PORT = Properties.Settings.Default.ListenPort;
        ...
        private void InitializeFormContextMenu()
        {
            ...
            // ── Изменить UDP-порт (встроенное поле ввода) ───────────────────────
            var portMenu = new ToolStripMenuItem($"{Localization.T("Change UDP Port")} ({LISTEN_PORT})");

            var portInput = new ToolStripTextBox
            {
                Text = LISTEN_PORT.ToString(),
                Width = 80
            };
            ...
            portInput.KeyDown += (s, e) =>
            {
                ...
                TryApplyPort(portInput.Text, portMenu);
            };

            portInput.LostFocus += (s, e) => TryApplyPort(portInput.Text, portMenu);

            portMenu.DropDownItems.Add(portInput);

            portMenu.DropDown.Closing += (s, e) =>
            {
                if (e.CloseReason == ToolStripDropDownCloseReason.ItemClicked) return;
                TryApplyPort(portInput.Text, portMenu);
            };

            formMenu.Items.Add(portMenu);
            formMenu.Items.Add(new ToolStripSeparator());
            formMenu.Items.Add(langMenu);

            // ← КРИТИЧНО: Присваиваем меню форме
        ...
        private void ApplyLanguageToAllMenus()
        {
            ...
            // Обновляем пункт порта в главном меню
            var portItem = this.ContextMenuStrip?.Items
                .OfType<ToolStripMenuItem>()
                .FirstOrDefault(i => i.DropDownItems.OfType<ToolStripTextBox>().Any());
            if (portItem != null)
            {
                portItem.Text = $"{Localization.T("Change UDP Port")} ({LISTEN_PORT})";
            }
        }
        #endregion
        ...
        {
            // Проверяем, что udpClient ещё живой
            if (udpClient == null) return;

            try
            {
                // Безопасно завершаем приём
                IPEndPoint? remoteEP = null;
                byte[]? data = udpClient.EndReceive(ar, ref remoteEP);

                // Сразу запускаем новый приём
                udpClient.BeginReceive(ReceiveCallback, null);

                // Проверяем данные
                if (data == null || data.Length != Marshal.SizeOf<GridLegendsMotionPacket189>()) return;
            ...
            {
                // Клиент закрыт — нормально, выходим
            }
        ...
        private void RestartUdpListener()
        {
            ...
            try
            {
                ipEndPoint = new IPEndPoint(IPAddress.Any, LISTEN_PORT);
                udpClient = new UdpClient(ipEndPoint);
                udpClient.BeginReceive(ReceiveCallback, null); // ← теперь безопасно
            }
            catch (Exception ex)
            {
                MessageBox.Show($"UDP Error: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void TryApplyPort(string input, ToolStripMenuItem menuItem)
        {
            if (!int.TryParse(input, out int newPort) || newPort < 1024 || newPort > 65535)
            {
                if (!string.IsNullOrWhiteSpace(input))
                {
                    MessageBox.Show(Localization.T("Invalid port! Must be 1024-65535."),
                        "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                return;
            }

            Properties.Settings.Default.ListenPort = newPort;
            Properties.Settings.Default.Save();
            LISTEN_PORT = newPort;
            RestartUdpListener();

            menuItem.Text = $"{Localization.T("Change UDP Port")} ({LISTEN_PORT})";
            foreach (ToolStripTextBox tb in menuItem.DropDownItems.OfType<ToolStripTextBox>())
                tb.Text = LISTEN_PORT.ToString();
        }
        ...
        private void UpdateUI(GridLegendsMotionPacket189 packet)
        {
            ...
            Set("RPM (Raw)", $"{packet.m_engineRPM * 250f:F0}");
