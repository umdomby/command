...
namespace TeleUDP
{
    public partial class Form1 : Form
    {
        ...
        private const int DefaultHiddenAlpha = 128; // 50% прозрачность
        ...
        private void Label_Paint(object? sender, PaintEventArgs e)
        {
            ...
            int textAlpha = tag != null && tag.ContainsKey(TagKeyAlpha) ? (int)tag[TagKeyAlpha] : 255;

            if (isOverlayMode && textAlpha == 0)
                return;
                ...
                using (Pen outlinePen = new Pen(Color.Black, 2))
                {
                    for (int x = -1; x <= 1; x++)
                    {
                        for (int y = -1; y <= 1; y++)
                        {
                            if (x != 0 || y != 0)
                            {
                                e.Graphics.DrawString(lbl.Text, lbl.Font, Brushes.Black,
                                    lbl.ClientRectangle.X + x, lbl.ClientRectangle.Y + y, new StringFormat
                                    {
            ...
            if (textAlpha > 0)
            {
                ...
                {
                    e.Graphics.DrawString(lbl.Text, lbl.Font, textBrush, lbl.ClientRectangle, new StringFormat
                    {
                        Alignment = StringAlignment.Center,
                        LineAlignment = StringAlignment.Center
                    });
                ...
                using (ColorDialog colorDialog = new ColorDialog())
                {
                    ...
                    if (colorDialog.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        targetLabel.Tag = new Dictionary<string, object>
                        {
                            ...
                            { TagKeyAlpha, (targetLabel.Tag as Dictionary<string, object>)![TagKeyAlpha] }
                        };
                        targetLabel.Invalidate();
                        SaveBlockPositions();
                    }
        ...
        private void ToggleVisible_Click(object? sender, EventArgs e)
        {
            ...
            {
                var tag = (targetLabel.Tag as Dictionary<string, object>)!;
                int currentAlpha = (int)tag[TagKeyAlpha];
                int newAlpha = currentAlpha > DefaultHiddenAlpha ? 0 : 255;

                tag[TagKeyAlpha] = newAlpha;
                targetLabel.Invalidate();
                SaveBlockPositions();
            }
        }
        ...
        private void ToggleVisibleAll_Click(object? sender, EventArgs e)
        {
            ...
            Label? firstVisible = GetDataLabels().FirstOrDefault(lbl => (int)(lbl.Tag as Dictionary<string, object>)![TagKeyAlpha] > DefaultHiddenAlpha);

            if (firstVisible != null)
                makeVisible = false;

            int targetAlpha = makeVisible ? 255 : DefaultHiddenAlpha;
                ...
                foreach (var blockData in data.Blocks)
                {
                    ...
                    if (lbl != null)
                    {
                        ...
                        lbl.Tag = new Dictionary<string, object>
                        {
                            ...
                            { TagKeyOutline, false },
                            { TagKeyAlpha, blockData.TextAlpha }
                        };
