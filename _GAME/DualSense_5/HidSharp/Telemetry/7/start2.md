...
при переходе в оверлей Полная прозрачность не передается прозрачность текста, а скорее всего передается оттенок, 
измени так чтобы прозрачность была в двух режимах.

нет не помогло, прозрачность блоков работает и цвет блоков работает а цвет текста и прозрачность текста не работает, возьми за пример как сделано с блоками сделай так и с текстом! дай измененный код целыми блоками

язык меняется только после перезагрузки и только у блоков, у общего меню (где ПКМ не по блоком, а по форме ПКМ) язык всегда остается русским


в общем меню ПКМ добавь выбор порта private const int LISTEN_PORT = 20778; чтобы можно было выбрать и добавить новый, порт так же добавь в английскую и русскую версию перевода