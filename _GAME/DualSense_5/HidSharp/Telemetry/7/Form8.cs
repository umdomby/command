...
namespace TeleUDP
{
    public partial class Form1 : Form
    {
        ...
        #region Paint (Красная рамка + 80 % текст)
        private void Label_Paint(object? sender, PaintEventArgs e)
        {
            ...
            // При скрытии в обычном режиме – 80 % прозрачность текста
            ...
            // Текст (всегда рисуется, но с нужной альфой)
            if (textAlpha > 0)
            {
        ...
        private void ToggleVisible_Click(object? sender, EventArgs e)
        {
            if (sender is ToolStripMenuItem menuItem)
            {
                ...
                {
                    lbl.Invalidate(); // перерисовка – рамка + 80 % текст
                }
        ...
        private void UpdateUI(GridLegendsMotionPacket189 packet)
        {
            void Set(string name, string text)
            {
                ...
                {
                    lbl.Text = text;
                    lbl.Invalidate(); // всегда перерисовываем – альфа будет 80 % если скрыт
                }
            }
