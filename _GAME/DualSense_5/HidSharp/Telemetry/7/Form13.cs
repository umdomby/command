...
namespace TeleUDP
{
    public partial class Form1 : Form
    {
        ...
        private readonly Color overlayColorKey = Color.Black; // Цвет, который будет прозрачным в оверлее
        ...
        private void ToggleOverlay_Click(object? sender, EventArgs e)
        {
            ...
            if (isOverlayMode)
            {
                ...
                this.BackColor = overlayColorKey; // Устанавливаем цвет, который будет прозрачным
                UpdateFormBackColor(); // Используем color key
            ...
            else
            {
                ...
                SetWindowLong(this.Handle, GWL_EXSTYLE, (style | WS_EX_LAYERED) & ~WS_EX_TRANSPARENT);
                this.BackColor = defaultFormBackColor;
                UpdateFormBackColor(); // Восстанавливаем альфа
