...
namespace TeleUDP
{
    public partial class Form1 : Form
    {
        ...
        private void ToggleOverlay_Click(object? sender, EventArgs e)
        {
            ...
            // <-- ИСПРАВЛЕНО: Принудительный перерисовка всех блоков -->
            foreach (Label lbl in GetDataLabels())
            {
                lbl.Invalidate();
            }
        ...
        // <-- ИСПРАВЛЕНО: Полное скрытие в оверлее -->
        private void Label_Paint(object? sender, PaintEventArgs e)
        {
            ...
            // В оверлее: если блок скрыт — НЕ РИСУЕМ НИЧЕГО
        ...
        private void ToggleVisible_Click(object? sender, EventArgs e)
        {
            ...
            if (targetLabel != null)
            {
                ...
                // <-- ИСПРАВЛЕНО: Принудительный Invalidate -->
                targetLabel.Invalidate();
                SaveBlockPositions();
            }
        }
