...
namespace TeleUDP
{
    public partial class Form1 : Form
    {
        ...
        private void InitializeFormContextMenu()
        {
            ...
            // <-- ИЗМЕНЕНО: Фон всех блоков — теперь с TrackBar -->
            ...
            backOpacityTrackBar.TrackBar.ValueChanged += (s, ev) =>
            {
                ...
                {
                    if (lbl.BackColor == Color.Transparent) continue;
                    Color newBack = Color.FromArgb(alpha, lbl.BackColor.R, lbl.BackColor.G, lbl.BackColor.B);
        ...
        private void InitializeDataBlocks()
        {
            ...
            // <-- УДАЛЕНО: пункт "Сделать фон прозрачным/непрозрачным" -->
        ...
        private void Label_Paint(object? sender, PaintEventArgs e)
        {
            ...
            // <-- ИЗМЕНЕНО: в оверлее скрытые блоки НЕ рисуются -->
        ...
        // <-- ИЗМЕНЕНО: скрытие/показ — теперь только альфа, без исчезновения -->
        ...
        private void ChangeBackColorAll_Click(object? sender, EventArgs e)
        {
            using (ColorDialog colorDialog = new ColorDialog())
            {
                if (colorDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach (Label lbl in GetDataLabels())
                    {
                        // Сохраняем текущую альфу, если она не 255
