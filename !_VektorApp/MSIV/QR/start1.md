...
### 
нужно переделать программу MSIV_QR, в новую SIV_QR_CLIENT_REAL сделать из программы только клиент, только на одну бочку только 72 (убрать выбор бочек), с показаниями контроллера 
и QR кода (под QR кодом ссылку) - сделай кнопку для админа, при нажатии на нее должен попросить пароль user1 при вводе его - должно открыться окно
для изменения ip ссылки - запроса.
дай код для SIV_QR_CLIENT_REAL с моими требованиями

###
       private readonly Barrel _barrel072 = new Barrel(
           "072", "ШАРДОНЕ в/м виноград. натур. белый сухой обраб.", "Ассамбляж", "94",
           5060.1, 4282.3, 777.8); сюда данные нужно вставлять из ответа из запроса 
string url = $"http://{AppSettings.ServerIpForQr}:/WEBKIP/hs/KIP/MZIV_Volume/071";
который приходит от сервера в формате HTML выведи данные пользователю, все осталное не трогай
