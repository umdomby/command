...
def main():
    ...
    print("\n" + "="*70)
    print("🔍 ПОЛНАЯ ДИАГНОСТИКА ONNX RUNTIME + GPU (RTX 5060)")
    print("="*70)
    ...
    try:
        ...
        # Проверка, что использует Ultralytics внутри
        if hasattr(model, 'model') and hasattr(model.model, 'session'):
            used_providers = model.model.session.get_providers()
            print(f"Модель YOLO использует       : {used_providers}")
            
            if "CUDAExecutionProvider" in used_providers:
                print("✅ УСПЕХ: YOLO работает на **GPU (CUDA)**")
            else:
                print("⚠️  YOLO работает на CPU (CUDA не удалось загрузить)")
        else:
            print("⚠️  Не удалось получить session модели")

        # Информация о PyTorch и GPU
        print(f"PyTorch CUDA доступен        : {torch.cuda.is_available()}")
        if torch.cuda.is_available():
            print(f"GPU название                 : {torch.cuda.get_device_name(0)}")
            print(f"Compute Capability           : {torch.cuda.get_device_properties(0).major}.{torch.cuda.get_device_properties(0).minor}")

    except Exception as diag_error:
        print("❌ Ошибка во время диагностики:")
        traceback.print_exc()

    print("="*70 + "\n")
    # =================================================================
    
    # Дополнительно — устройство
    print("CUDA доступен в PyTorch:", torch.cuda.is_available())
    print("Текущий device модели:", model.device if hasattr(model, 'device') else "неизвестно")
    h_cam = ueye.HIDS(0)
