...
const CMD_HEARTBEAT     = 0x10  // Не используется для моторов
const CMD_HBT_MOTOR     = 0x11  // Новый для моторов
...
export default function SocketClient() {
    ...
    const handleMotorControl = useCallback((motor: 'A'|'B', value: number) => {
        ...
        // Для остальных джойстиков — троттлинг 40 мс
    ...
    // HBT_MOTOR каждые 300 мс при активных моторах
    useEffect(() => {
        if (!isConnected || !isIdentified || (motorASpeed <= 0 && motorBSpeed <= 0)) return

        const interval = setInterval(sendHbtMotor, 300)
        return () => clearInterval(interval)
    }, [isConnected, isIdentified, motorASpeed, motorBSpeed, sendHbtMotor])
