...
export default function SocketClient() {
    ...
    const [motorsActive, setMotorsActive] = useState(false);
    ...
    // Последние отправленные значения для JoyAnalog (для гистерезиса ±2)
    const lastSentValueA = useRef(0)
    const lastSentValueB = useRef(0)

    const [motorASpeed, setMotorASpeed] = useState(0)
    const [motorBSpeed, setMotorBSpeed] = useState(0)
    const [motorADirection, setMotorADirection] = useState<0|1|2>(0)
    ...
    useEffect(() => {
        const active = motorASpeed > 0 || motorBSpeed > 0;
        setMotorsActive(active);
    }, [motorASpeed, motorBSpeed]);

    const cleanupWebSocket = useCallback(() => {
        if (socketRef.current && socketRef.current.readyState === WebSocket.OPEN) {
            const stopA = new Uint8Array([CMD_MOTOR, 65, 0, 0]);
            const stopB = new Uint8Array([CMD_MOTOR, 66, 0, 0]);
            socketRef.current.send(stopA);
            socketRef.current.send(stopB);
            addLog('Моторы остановлены при отключении', 'info');
        }
    ...
    const connectWebSocket = useCallback(() => {
        ...
        ws.onmessage = (event) => {
            ...
            if (type === RSP_FULL_STATUS && data.length >= 9) {
                ...
                const voltage = voltRaw * 0.021888
    ...
    const sendHbtMotor = useCallback(() => {
        sendBinary(new Uint8Array([CMD_HBT_MOTOR]))
    }, [sendBinary])
    ...
    const handleMotorControl = useCallback((motor: 'A'|'B', value: number, isFromGamepad = false) => {
        ...
        // Гистерезис ±2 только для JoyAnalog
        if (isFromGamepad) {
            const lastSent = motor === 'A' ? lastSentValueA.current : lastSentValueB.current
            if (Math.abs(value - lastSent) < 3 && speed !== 0) {
                return
            }
            if (motor === 'A') lastSentValueA.current = value
            else lastSentValueB.current = value
        }

        // Мгновенная остановка при 0
        if (speed === 0) {
            sendMotor(motor, 0, 0)
            return
        }

        // Для JoyAnalog — мгновенная отправка
        if (isFromGamepad) {
            sendMotor(motor, speed, dir)
            return
        }

        // Для остальных — троттлинг 40 мс
    ...
    const handleDualAxisControl = useCallback(({ x, y }: { x: number; y: number }) => {
        handleMotorControl('A', x, true)  // true = JoyAnalog
        handleMotorControl('B', y, true)
    }, [handleMotorControl])

    const handleOtherJoystick = useCallback((motor: 'A'|'B', value: number) => {
        handleMotorControl(motor, value, false)
    }, [handleMotorControl])
    ...
    useEffect(() => {
        if (!isConnected || !isIdentified || !motorsActive) {
            return;
        }
                ...
                {(selectedJoystick === 'Joystick' || selectedJoystick === 'JoystickUp') && (
                    <>
                        {selectedJoystick === 'Joystick' ? (
                            <Joystick
                                mo="A"
                                onChange={(v) => handleOtherJoystick('A', v)}
                                disabled={disabled}
                        ...
                        {selectedJoystick === 'Joystick' ? (
                            <Joystick
                                mo="B"
                                onChange={(v) => handleOtherJoystick('B', v)}
                                disabled={disabled}
