...
export const HEROES_CLIENT: React.FC<Props> = ({ className, user }) => {
    ...
    const calculateMaxBet = (bet: Bet, player: PlayerChoice) => {
        ...
        // Максимальная ставка, чтобы не превысить сумму другого игрока на 100%
    ...
    const handlePlaceBet = async (bet: Bet, amount: number, player: PlayerChoice) => {
        try {
            ...
            // Проверка, не превышает ли ставка сумму другого игрока на 100%
            if (player === PlayerChoice.PLAYER1 && amount > totalPlayer2 * 2) {
                throw new Error('Ставка превышает сумму другого игрока на 100%');
            }

            if (player === PlayerChoice.PLAYER2 && amount > totalPlayer1 * 2) {
                throw new Error('Ставка превышает сумму другого игрока на 100%');
            }
                        ...
                        {bet.status === 'OPEN' && (
                            <div className="mt-4">
                                <p>
                                    ...
                                    <span className={playerColors[PlayerChoice.PLAYER1]}>{calculateMaxBet(bet, PlayerChoice.PLAYER1).toFixed(2)}</span>
                                </p>
                                <p>
                                    Максимальная ставка на <span className={playerColors[PlayerChoice.PLAYER2]}>{bet.player2.name}</span>:{' '}
                                    <span className={playerColors[PlayerChoice.PLAYER2]}>{calculateMaxBet(bet, PlayerChoice.PLAYER2).toFixed(2)}</span>
                                </p>
                            </div>
                        )}
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            ...
                                            if (isNaN(value) || value <= 0) {
                                                ...
                                                const maxAllowedBet = calculateMaxBet(bet, selectedPlayer as PlayerChoice);
