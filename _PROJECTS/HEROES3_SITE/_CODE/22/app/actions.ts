...
export async function placeBet(formData: { betId: number; userId: number; amount: number; player: PlayerChoice }) {
  try {
    ...
    const maxBetForProfitPlayer1 = (totalPlayer2 * 0.3) / oddsPlayer1;
    const maxBetForProfitPlayer2 = (totalPlayer1 * 0.3) / oddsPlayer2;
    ...
    const maxAllowedBetPlayer1 = Math.min( maxBetForProfitPlayer1, userMaxBet);
    const maxAllowedBetPlayer2 = Math.min( maxBetForProfitPlayer2, userMaxBet);
