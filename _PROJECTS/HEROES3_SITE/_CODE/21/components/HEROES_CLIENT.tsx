...
export const HEROES_CLIENT: React.FC<Props> = ({ className, user }) => {
    ...
    const [odds, setOdds] = useState<{ [key: number]: { player1: number; player2: number } }>({});


    useEffect(() => {
        if (bets) {
            const initialErrors: { [key: number]: string | null } = {};
            bets.forEach(bet => {
                initialErrors[bet.id] = null;
            });
            setPlaceBetError(initialErrors);
        }
    }, [bets]);
    ...
    const handlePlaceBet = async (bet: Bet, amount: number, player: PlayerChoice) => {
        try {
            ...
            setOdds((prev) => ({
                ...prev,
                [bet.id]: { player1: oddsPlayer1, player2: oddsPlayer2 },
            }));
            ...
            setPlaceBetError((prev) => ({
                ...prev,
                [bet.id]: null, // Сбрасываем ошибку для этой ставки
            }));
        } catch (err) {
            if (err instanceof Error) {
                setPlaceBetError((prev) => ({
                    ...prev,
                    [bet.id]: err.message, // Устанавливаем ошибку для этой ставки
                }));
            } else {
                setPlaceBetError((prev) => ({
                    ...prev,
                    [bet.id]: 'Неизвестная ошибка', // Устанавливаем ошибку для этой ставки
                }));
            }
                ...
                {calculateMaxBet(bet, PlayerChoice.PLAYER2).toFixed(2)}
                        ...
                        {bet.status === 'OPEN' && (
                            <div>
                                ...
                                {placeBetError && placeBetError[bet.id] && <p className="text-red-500">{placeBetError[bet.id]}</p>}
                            </div>
                        )}
