...
export const HEROES_CLIENT: React.FC<Props> = ({ className, user }) => {
    ...
    const [hasPlacedBet, setHasPlacedBet] = useState<{ [key: number]: boolean }>({});
    const [oddsErrors, setOddsErrors] = useState<{ [key: number]: string | null }>({}); // Новое состояние для ошибок коэффициента
    ...
    const handleValidation = (bet: Bet, amount: number, player: PlayerChoice) => {
        ...
        const maxAllowedBetValue = player === PlayerChoice.PLAYER1 ? bet.maxBetPlayer1 : bet.maxBetPlayer2;

        // Валидация коэффициента
        if (currentOdds <= MIN_ODDS) {
            setOddsErrors((prev) => ({
                ...prev,
                [bet.id]: `Коэффициент ставки (${currentOdds.toFixed(2)}) ниже или равен минимальному допустимому значению (${MIN_ODDS})`,
            }));
        ...
        // Валидация максимальной ставки
        if (amount > maxAllowedBetValue) {
            setPlaceBetErrors((prev) => ({
                ...prev,
        ...
        // Если обе валидации пройдены, разблокируем кнопку
        if (currentOdds > MIN_ODDS && amount <= maxAllowedBetValue) {
            setIsBetDisabled((prev) => ({
                ...prev,
                [bet.id]: false,
            }));
