...
export const HEROES_CLIENT: React.FC<Props> = ({ className, user }) => {
    ...
    const calculateMaxBet = (bet: Bet, player: PlayerChoice) => {
        ...
        const oddsPlayer1 = totalBetPlayer1 === 0 ? 1 : totalBets / totalBetPlayer1;
        const oddsPlayer2 = totalBetPlayer2 === 0 ? 1 : totalBets / totalBetPlayer2;

        // Максимальная ставка, чтобы коэффициент не опустился ниже 1.05
        const maxBetForOdds = player === PlayerChoice.PLAYER1
            ? Math.max((totalBets / 1.05) - totalBetPlayer1, 0) // Не меньше 0
            : Math.max((totalBets / 1.05) - totalBetPlayer2, 0); // Не меньше 0

        // Максимальная ставка, чтобы прибыль не превышала 30% от суммы ставок на другого игрока
        const maxBetForProfit = player === PlayerChoice.PLAYER1
            ? (totalBetPlayer2 * 0.3) / oddsPlayer1 // Прибыль = ставка * коэффициент
            : (totalBetPlayer1 * 0.3) / oddsPlayer2;
        ...
        return Math.min(maxBetForOdds, maxBetForProfit, userMaxBet);
    };
    ...
    const handlePlaceBet = async (bet: Bet, amount: number, player: PlayerChoice) => {
        try {
            ...
            // Возвращаем минимальное значение из всех ограничений
            const maxAllowedBet = Math.min(maxBetForOdds, maxBetForProfit, userMaxBet);
            ...
            setMaxBets((prev) => ({
                ...prev,
                [bet.id]: {
                    player1: calculateMaxBet(bet, PlayerChoice.PLAYER1),
                    player2: calculateMaxBet(bet, PlayerChoice.PLAYER2),
                },
            }));
