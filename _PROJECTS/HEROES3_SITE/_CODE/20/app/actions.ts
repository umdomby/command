...
export async function placeBet(formData: { betId: number; userId: number; amount: number; player: PlayerChoice }) {
  try {
    ...
    // Проверка на дублирование ставок
    const existingParticipant = await prisma.betParticipant.findFirst({
      where: {
        betId,
        userId,
        player, // Проверяем, что пользователь уже сделал ставку на этого игрока
      },
    });

    if (existingParticipant) {
      throw new Error('Вы уже сделали ставку на этого игрока в этой ставке');
    }
    ...
    const maxBetForProfit = player === PlayerChoice.PLAYER1
        ? (totalPlayer2 * 0.3) / oddsPlayer1
        : (totalPlayer1 * 0.3) / oddsPlayer2;
