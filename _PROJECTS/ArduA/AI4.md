...
эти библиотеки используются их не меняй
...
если ведомый хочет подсоединиться к комнате, а комнаты нету то идет оповещение, что комнаты нет. Оповещение не акак ошибка, а как оповещение!!!
если ведомый хочет присоединиться к комнате, а там уже есть ведомый и ведущий, комната удаляется, ведущий автоматически создает новую комнату и ждет нового соединения с ведомым.
если комната есть и в ней нет ведомых, ведомый присоединяется к комнате
если ведущий хочет присоединиться к комнате которая уже создана, то комната которая создана удаляется и создается новая.
...
Ведущий и ведомый не создается видеотрансляция.
У меня сколько я раз не создавал, очень частая ошибка переподключения, в комнату заходит, но не всегда ведущий начинает передавать видео ведомому
тестирую я в локальной сети!
Нужно сделать обязательно, переподключение android ведущего - если связь с сервером Go потеряна. - сделать вечный цикл переподключения.
После как любой ведомый отключается - полностью очищать данные для соединения и получения видео от ведущего. Может даже лучше будет - когда ведомый отсоединяется, комната закрывается и ведущий создает новую комнату для соединения с ведомым, так лучше будет для соединений если комната будет каждый раз пересоздаваться ведущим да ICE кандидатов?

дай абсолютно полные кода каждого файла.
...
Ведущий создает новую комнату при следующем подключении


когда ведомый уходит из комнаты, комната закрывается, а android заново не соединяется сервером и не создает комнату,
нужно чтобы андройд отслеживал соединение и если соединение разрывается андройд - ведущий соединялся с сервером и создавал комнату для конференции каждый раз. Может ping pong настроить?

Проблема в том, что когда ведущий (лидер) переподключается после разрыва соединения, он не автоматически создает новый оффер. Давайте исправим это, добавив логику автоматического создания оффера при переподключении.
