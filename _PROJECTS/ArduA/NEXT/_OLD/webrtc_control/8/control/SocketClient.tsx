...
export default function SocketClient({ compactMode, onStatusChange }: SocketClientProps) {
    ...
    const connectWebSocket = useCallback((deviceIdToConnect: string) => {
        ...
        ws.onopen = () => {
            ...
            useMotorControl.getState().setSocket(ws);
        ...
        ws.onclose = (event) => {
            ...
            //setSendCommandHandler красный?
            useMotorControl.getState().setSendCommandHandler((cmd, params) => {
                if (socketRef.current?.readyState === WebSocket.OPEN) {
                    socketRef.current.send(JSON.stringify({ command: cmd, params }));
                }
            });
