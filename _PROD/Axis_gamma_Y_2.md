в iphone13 в горизонтальном положении экраном вертикально (экран смотрит пользователю) если крутить перед собой ось Y(gamma)
...
4 перехода : ...[-87,-88,-89 (переход) 89,88,87]...[3,2,1,0 (переход)-0,-1,-2]...[-87,-88,-89 (переход)...89,88,87]...[3,2,1,0(переход)...-0,-1,-2,-3...] если крутить перед собой ось Y(gamma) и телефон в горизонтальном положении
нужно отправлять данные на servo1 который должен получать только данные в диапозоне [0...180] градусов, функция не должна отправлять данные со знаком минус и более 180.
90 градусов servo1 возьмем этот переход [-0...-87,-88,-89 (переход) 89,88,87...0],  берем за [90 servo1 градусов] = [89 gamma]
0 градусов возьмем -0
179-180 градусов возьмем 0

также обозначим мертвые зоны (переход) Y(gamma) где servo1 не должен получать данные от Y(gamma)
...[3,2,1,0 (переход)-0,-1,-2]...
когда servo1 < 90 то эти данные 3,2,1,0  на servo1 не отправляются
когда servo1 > 90 то эти данные -0,-1,-2 на servo1 не отправляются
отвечай на русском, дай полный код с комментариями

###
не правильно работают мертвые зоны при переходе ...[3,2,1,0 (переход)-0,-1,-2]...
также обозначим мертвые зоны (переход) Y(gamma) где servo1 не должен получать данные от Y(gamma)
...[3,2,1,0 (переход)-0,-1,-2]...
когда servo1 < 90 то эти данные 3,2,1,0  на servo1 не отправляются потому что за 0 servo1 не надо отправлять данные!
когда servo1 > 90 то эти данные -0,-1,-2 на servo1 не отправляются потому что за 180 servo1 не надо отправлять данные!
сделай чтобы по достижению sero1 крайних границ (троеточие) ...0 и 180... servo1 не получал данные от gamma  после 0 и после 180
нет, ты не исправил, servo1 получает данные от gamma когда servo1 ....данные...0 и 180...данные.... и servo1 крутится как долбоеб
отвечай на русском, дай полный код с комментариями
