...
export default function UseNoRegWebRTC({ roomId, setLeaveRoom, videoTransform, setWebSocket, useBackCamera }: NoRegWebRTCProps) {
    ...
    const VIDEO_CHECK_TIMEOUT = 10000; // Увеличено до 10 секунд
    ...
    const checkNetwork = () => {
        if (!navigator.onLine) {
            console.warn('Нет подключения к сети');
            return false;
        }
        if ('connection' in navigator && navigator.connection) {
            const connection = navigator.connection as any;
            if (connection.effectiveType === '2g' || connection.downlink < 0.5) {
                console.warn('Слабая сеть (2G или скорость < 0.5 Mbps), откладываем подключение');
                return false;
            }
        }
        return true;
            ...
            const attemptConnection = () => {
                ...
                try {
                    ...
                    const onClose = (event: CloseEvent) => {
                        ...
                        if (setWebSocket) setWebSocket(null);
                        if (event.code === 1006) {
                            console.warn('WebSocket закрыт с кодом 1006, возможны проблемы с сетью, сертификатом или брандмауэром');
                        }
                        if (retryCount < maxRetries) {
                            retryCount++;
                            const backoffDelay = Math.min(10000 * Math.pow(2, retryCount), 30000);
    ...
    const initializeWebRTC = async () => {
        ...
        pc.current = new RTCPeerConnection({
            ...
            // sdpSemantics удалено, старые браузеры используют plan-b по умолчанию
            // Для явной поддержки plan-b можно добавить полифилл, например, adapter.js
        });
