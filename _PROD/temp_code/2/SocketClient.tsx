    ...
    const createMotorHandler = useCallback((mo: 'A' | 'B') => {
        ...
        return (value: number) => {
            ...
            if (selectedJoystick === 'JoystickTurn') {
                // Для JoystickTurn: Y управляет скоростью, X управляет поворотом
                if (mo === 'A') {
                    // Мотор A: Y для скорости, X для поворота (левый мотор замедляется при повороте вправо)
                    sp = Math.abs(value); // Y для скорости
                    if (value > 0) {
                        direction = 'forward';
                    } else if (value < 0) {
                        direction = 'backward';
                    ...
                    // Мотор B: Y для скорости, X для поворота (правый мотор замедляется при повороте влево)
                    sp = Math.abs(value); // Y для скорости
                    if (value > 0) {
                        direction = 'forward';
                    } else if (value < 0) {
                ...
                // Логика для других джойстиков (Joystick, JoystickUp)
                if (value > 0) {
                    direction = 'forward';
                    sp = value;
                } else if (value < 0) {
    ...
    return (
        <div className="flex flex-col items-center min-h-[calc(100vh-3rem)] p-4 overflow-hidden relative">
            ...
            <div className={`mt-24 ${activeTab === 'controls' ? 'opacity-50' : ''}`}>
                <ActiveJoystick
                    onChange={(x, y) => {
                        handleMotorAControl(y) // Y управляет скоростью (вперёд/назад)
                        handleMotorBControl(x) // X управляет поворотом (лево/право)
                    }}
                    disabled={!isConnected}
                />
