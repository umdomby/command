...
export default function UseNoRegWebRTC({ roomId, setLeaveRoom, videoTransform, setWebSocket, useBackCamera }: NoRegWebRTCProps) {
    ...
    const MAX_RETRIES = 5;
    const VIDEO_CHECK_TIMEOUT = 15000;
    const WS_TIMEOUT = 12000; // Увеличен для медленных сетей
    const MAX_JOIN_MESSAGE_RETRIES = 5;
    ...
    const connectWebSocket = async (): Promise<boolean> => {
        return new Promise((resolve) => {
            ...
            if (!navigator.onLine) {
                ...
                console.error('connectWebSocket: Нет подключения к интернету');
                resolve(false);
                return;
            }
            ...
            let retryCount = 0;
            const { isOldAndroid } = detectPlatform();
            const wsUrl = isOldAndroid
                ? (process.env.WEBSOCKET_URL_WSGO || 'ws://ardua.site:444/wsgo') // ws:// для старых устройств
                : (process.env.WEBSOCKET_URL_WSGO || 'wss://ardua.site:444/wsgo');

            const retryOrFallback = (event?: Event) => {
                if (retryCount < MAX_RETRIES) {
                    retryCount++;
                    const delay = Math.min(2000 * Math.pow(2, retryCount), 15000); // Увеличена базовая задержка
                    console.log(`connectWebSocket: Повторная попытка через ${delay} мс, попытка ${retryCount}/${MAX_RETRIES}`);
                    setTimeout(() => attemptConnection(), delay);
                } else {
                    const errorMessage = event instanceof CloseEvent
                        ? `WebSocket закрыт: код ${event.code}${event.reason ? `, причина: ${event.reason}` : ''}`
            ...
            const attemptConnection = () => {
                ...
                try {
                    ...
                    const onOpen = () => {
                        ...
                        pingIntervalRef.current = setInterval(() => {
                            if (ws.current?.readyState === WebSocket.OPEN) {
                                ...
                                console.warn('connectWebSocket: WebSocket закрыт, остановка ping');
                    ...
                    const onClose = (event: CloseEvent) => {
                        console.log('connectWebSocket: WebSocket закрыт:', {
                            code: event.code,
                            reason: event.reason,
    ...
    const initializeWebRTC = async () => {
        ...
        pc.current = new RTCPeerConnection({
            ...
            iceTransportPolicy: isOldAndroid ? 'relay' : 'all',
    ...
    const joinRoom = async () => {
        ...
        try {
                ...
                const onMessage = (event: MessageEvent) => {
                    try {
                        ...
                        if (data.type === 'room_info') {
                            ...
                            setError(`Ошибка: ${data.data || 'Неизвестная ошибка'}`);
                            cleanupEvents();
                            if (joinMessageRetries.current < MAX_JOIN_MESSAGE_RETRIES) {
                                ...
                                reject(new Error(data.data || 'Неизвестная ошибка'));
                            }
                        }
                    } catch (err) {
            ...
            if (retryAttempts.current < MAX_RETRIES && !isConnectionStable.current) {
                console.log('joinRoom: Планируем повтор через 7 сек');
                webRTCRetryTimeoutRef.current = setTimeout(() => {
                    retryAttempts.current += 1;
                    setRetryCount(retryAttempts.current);
                    if (!isReconnecting.current && !isConnectionStable.current) resetConnection();
                }, 7000); // Увеличена задержка
            } else {
    ...
    const resetConnection = () => {
        const now = Date.now();
        if (isReconnecting.current || now - lastRetryTimestamp.current < 7000) {
