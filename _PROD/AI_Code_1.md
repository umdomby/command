    ...
    private fun findAvailableFlashlightCamera(): String? {
        try {
            for (id in cameraManager.cameraIdList) {
                ...
                if (hasFlash && isCameraAvailable(id) && checkFlashlightSupport(id)) {
                    return id
                }
            }
        } catch (e: CameraAccessException) {
    ...
    private fun toggleFlashlight() {
        flashlightCameraId?.let { cameraId ->
            if (!isCameraAvailable(cameraId)) {
                Log.w("WebRTCService", "Камера недоступна для включения фонарика")
                return
            }

            try {
                // 1. Приостанавливаем видеозахват
                webRTCClient.videoCapturer?.stopCapture()

                // 2. Даем камере время освободиться (300-500мс)
                handler.postDelayed({
                    try {
                        ...
                        // 3. Возобновляем видеозахват с небольшой задержкой
                        handler.postDelayed({
                            try {
                                webRTCClient.videoCapturer?.startCapture(640, 480, 30)
                            } catch (e: Exception) {
                                Log.e("WebRTCService", "Ошибка при перезапуске видеопотока", e)
                                reconnect() // Попытка восстановить соединение
                            }
                        }, 300)
                    } catch (e: CameraAccessException) {
                        Log.e("WebRTCService", "Ошибка управления фонариком: ${e.message}")
                        // Если не удалось переключить фонарик, все равно возобновляем видеозахват
                        handler.postDelayed({
                            webRTCClient.videoCapturer?.startCapture(640, 480, 30)
                        }, 300)
                    }
                ...
                Log.e("WebRTCService", "Ошибка при остановке видеозахвата", e)
            }
