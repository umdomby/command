...
// Преобразуем telegramId в BigInt для базы данных
const updateData = {
...settings,
telegramId: settings.telegramId !== null && settings.telegramId !== undefined
? BigInt(settings.telegramId) // Prisma ожидает BigInt
: null,
};
...
model Settings {  servoView      Boolean
{showServos - должен отображаться при первой загрузке страницы -   от model Settings  servoView Boolean этих параметров - беруться из серверноый функций actions,
сейчас {showServos всегда загружается открытым,

из VideoCallApp наверное берутся данные из model Settings  servoView но в SocketClient не передается первоначальная загрузка {showServos и она постоянно true
