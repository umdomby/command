...
interface JoyAnalogProps {
    ...
    onRelayChange?: (pin: string, state: string) => void, // Новый пропс
    disabled?: boolean,
}
...
const JoyAnalog = ({onChange, onServoChange, disabled, onServoChangeCheck, onRelayChange }: JoyAnalogProps) => {
    ...
    const prevButtonState = useRef({
        ...
        buttonLB: false,
        buttonRB: false,
        buttonMenu: false, // Новый флаг для кнопки Menu
    });
    const prevStickState = useRef({
        leftStickX: 0, // Для Servo2 (D8, SSR2)
        rightStickY: 0, // Для Servo1 (D7, SSR)
        servo1Value: 90, // Предыдущее значение для Servo1
        servo2Value: 90, // Предыдущее значение для Servo2
    });

    // Проверка подключения геймпада
    const checkGamepad = useCallback(() => {
        const gamepads = navigator.getGamepads();
        const gamepad = gamepads.find((gp) => gp !== null && (gp.id.includes("Xbox") || gp.mapping === "standard"));
    ...
    const handleGamepadInput = useCallback(() => {
        ...
        // D-Pad (Крестовина)
        const dpadDown = gamepad.buttons[12].pressed; // Вверх
        const dpadUp = gamepad.buttons[13].pressed; // Вниз
        ...
        // Кнопка Menu (индекс 8)
        const buttonMenu = gamepad.buttons[8].pressed; // Menu (справа от кнопки Xbox)
        ...
        // Переключение реле D0 при нажатии кнопки Menu
        if (buttonMenu && !prevButtonState.current.buttonMenu) {
            console.log(`Button Menu pressed, toggling relay D0`);
            onRelayChange?.("D0", "toggle"); // Используем "toggle" для указания переключения состояния
        }

        // Левый стик (Servo2, ось X)
        if (Math.abs(leftStickX) > deadZone) {
            const servo2Value = Math.round((-leftStickX + 1) * 90); // От 0 до 180, инвертировано
            if (servo2Value !== prevStickState.current.servo2Value) {
                onServoChange("2", servo2Value, true);
                prevStickState.current.servo2Value = servo2Value;
            }
        } else if (Math.abs(leftStickX) <= deadZone && Math.abs(prevStickState.current.leftStickX) > deadZone) {
            if (prevStickState.current.servo2Value !== 90) {
                onServoChange("2", 90, true); // Возврат в центр (90°)
                prevStickState.current.servo2Value = 90;
            }
        }
        ...
        if (Math.abs(rightStickY) > deadZone) {
            ...
            if (servo1Value !== prevStickState.current.servo1Value) {
                onServoChange("1", servo1Value, true);
                prevStickState.current.servo1Value = servo1Value;
            }
        } else if (Math.abs(rightStickY) <= deadZone && Math.abs(prevStickState.current.rightStickY) > deadZone) {
            if (prevStickState.current.servo1Value !== 90) {
                onServoChange("1", 90, true); // Возврат в центр (90°)
                prevStickState.current.servo1Value = 90;
            }
        }
        ...
        prevButtonState.current = {
            ...
            buttonLB,
            buttonRB,
            buttonMenu, // Добавляем buttonMenu
        };
        prevStickState.current = {
            leftStickX,
            rightStickY,
            servo1Value: prevStickState.current.servo1Value,
            servo2Value: prevStickState.current.servo2Value,
        };

        // Продолжаем опрос геймпада
        animationFrameRef.current = requestAnimationFrame(handleGamepadInput);
    }, [disabled, checkGamepad, onChange, onServoChange, onServoChangeCheck, onRelayChange]);
